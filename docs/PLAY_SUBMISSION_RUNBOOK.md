...
- [docs/PLAY_ASSET_SPEC.md](docs/PLAY_ASSET_SPEC.md)
...
```text
Catholic fasting and abstinence with reminders and local daily guidance.
```
...
- Use [docs/PLAY_ASSET_SPEC.md](docs/PLAY_ASSET_SPEC.md) as the source of truth for file specs, naming, screenshot order, prompt guardrails, and truthfulness rules.
...
## Release Diagnostic Uploads

If Play warns about missing crash-diagnostic files for the release bundle, upload:

- Deobfuscation file: `app/build/outputs/mapping/release/mapping.txt`
- Native debug symbols, only if Gradle generated the archive:
  `app/build/outputs/native-debug-symbols/release/native-debug-symbols.zip`

These files are generated by the release build and are not store-listing assets.
